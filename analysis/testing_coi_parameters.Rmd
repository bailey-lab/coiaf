...
# %>% ggpubr::ggexport(filename = "Sensitivity Analysis Figures/tcoi.png")
```
...
tdistance <- coi_test(COI = 2:10, PLAF = p, method = "overall",
...
tdistance_error <- error_plot(tdistance, fill = "dist_method", 
                              ...
                              fill_levels = c("Absolute Sum", "Sum of Absolute",
                                        "Squared Error", "KL Divergence"),
...
tweight <- coi_test(COI = 2:10, weighted = c(FALSE, TRUE),
                    dist_method = c("abs_sum", "sum_abs",
                                      "squared", "KL"),
                    PLAF = p, repetitions = 10)
...
tcoverage <- coi_test(COI = 2:10, coverage = c(25, 50, 100, 200, 400, 800), 
...
bloci <- lapply(loci, function(new_L){
  ...
  inner_tloci <- coi_test(COI = 2:20, repetitions = 10, PLAF = new_p)
...
for (i in seq_len(length(update_images))){
  filename = paste0("analysis/Sensitivity Analysis Figures/Sensitivity Plots/", 
...
for (i in seq_len(length(update_errors))){
  filename = paste0("analysis/Sensitivity Analysis Figures/Error Images/", 
...
<!-- # ```{r} -->
<!-- # test <- tdistance$boot_error %>% tidyr::unnest(cols = names(tdistance$boot_error)) -->
<!-- # test <- tcoi$boot_error %>% tidyr::unnest(cols = names(tcoi$boot_error)) -->
<!-- # test <- tmethod$boot_error %>% tidyr::unnest(cols = names(tmethod$boot_error)) -->
<!-- #  -->
<!-- # test <- test %>%  -->
<!-- #   dplyr::mutate(dplyr::across(where(function(x) dplyr::n_distinct(x) > 1) &  -->
<!-- #                   !all_of(c("mae", "lower", "upper", "bias")), as.factor)) %>% -->
<!-- #   dplyr::mutate(mae = dplyr::na_if(mae, 0)) %>%  -->
<!-- #   dplyr::mutate(lower = dplyr::na_if(lower, 0)) %>%  -->
<!-- #   dplyr::mutate(upper = dplyr::na_if(upper, 0)) -->
<!-- #  -->
<!-- # library(ggplot2) -->
<!-- # ggplot(test, aes(x = COI, y = mae, fill = method)) + -->
<!-- #   geom_col(position=position_dodge(), na.rm = T) + -->
<!-- #   geom_errorbar(aes(ymin = lower, ymax = upper), width = .2, position=position_dodge(.9)) + -->
<!-- #   theme_classic() + -->
<!-- #   theme(legend.position = "bottom") + -->
<!-- #   labs(y = "Mean Absolute Error", fill = "Distance Method") -->
<!-- #  -->
<!-- # error_plot(tmethod, fill = "method", legend_title = "Method") -->
<!-- # ``` -->
