...
```{r plot all data}
label_names <- c("continuous" = "Continuous Optimization",
...
ggplot(pivot_data, aes(y = COI, x = rmcl_med, color = coi_method)) +
  ...
  ggplot2::geom_count(alpha = 0.7, position=position_jitter(w = 0.1)) +
  mytheme +
...
# Only discrete
ggplot(dplyr::filter(pivot_data, optimization == "discrete"),
...
# Only continuous
ggplot(dplyr::filter(pivot_data, optimization == "continuous"),
...
map_average <- patient_lat_long %>% 
  ...
  dplyr::summarise(coi_mean = mean(discrete_1_med),
...
# Plot the mean COI
mean_world <- world_map(map_average, map_average$coi_mean, label = "Mean COI", 
                        alpha = 0.7, breaks = c(1.0, 1.5, 2.0, 2.5))

# Plot the median COI
med_world <- world_map(map_average, map_average$coi_med, label = "Median COI", 
...
prev_fws <- patient_lat_long %>%
  ...
  dplyr::mutate(prev = as.numeric(prev_2_10), 
...
grouped <- prev_fws %>% 
  ...
  dplyr::summarise(COI = median(COI),
                   Fws = median(Fws),
                   .groups = "drop") 
...
ggplot(data = tidyr::drop_na(grouped), 
       aes(x = cut(prev, seq(0, 1, 0.1), right = TRUE), 
           y = COI, color = Method)) +
