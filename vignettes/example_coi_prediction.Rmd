...
level allele frequency (PLMAF) using a beta distribution.
...
for (k in 2:7) {
  ...
  # Plot raw WSMAF as gray dots
  plot(sim1$data$plmaf, sim1$data$wsmaf,
    pch = 20, cex = 0.5, col = "gray", ylim = c(0, 1),
    xlab = "PLMAF", ylab = "WSMAF", main = paste("COI =", k)
  )

  # Overlay analytical expectations for COI of 2:7
  p_vec <- seq(0, 0.5, length.out = 101)
  ...
  # Compute windowed averages over WSMAF and overlay in blue
...
In this, the intercept at `PLMAF = 0.5` gives us an easy way to estimate COI,
with `COI = 1 + log_0.5(1-WSMAF)`, or `WSMAF = 1 - 0.5^(COI-1)`. However, the
differences between each COI at PLMAF = 0.5 become geometrically smaller. Larger
gaps can be found at different COI, with smaller PLMAF:
...
In method 2, we plot WSMAF against PLMAF for only the sites that are
...
for (k in 2:7) {
  ...
  # plot raw WSMAF as grey dots
  plot(sim1$data$plmaf, sim1$data$wsmaf,
    pch = 20, cex = 0.5, col = "gray",
    ylim = c(0, 1), xlab = "PLMAF", ylab = "WSMAF", main = paste("COI =", k)
  )
  ...
  # compute sliding-window averages over WSMAF and overlay in blue
...
In this case, COI is found as 1/WSMAF when PLMAF = 0 (i.e. the y-axis intercept).
