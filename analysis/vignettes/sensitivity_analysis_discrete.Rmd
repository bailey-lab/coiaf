...
toverall <- sensitivity(coi = 1:20, repetitions = 100, plaf = p, 
                        coverage = 1000, seq_error = 0, 
                        coi_method = c("1", "2"))

toverall_image <- sensitivity_plot(data = toverall, dims = c(1, 2), 
                                   result_type = "disc",
                                   title = "Predicted COI", 
...
tcoi <- sensitivity(coi = 2:40, max_coi = 40, repetitions = 100, plaf = p,
                    coi_method = c("1", "2"), seq_error = 0.01)

tcoi_image <- sensitivity_plot(data = tcoi, dims = c(1, 2), 
                               result_type = "disc",
                               title = "Predicted COI", 
                               sub_title = c("Method 1", "Method 2"))
...
tcoi_fig <- ggpubr::ggarrange(tcoi_image, tcoi_error, 
                              labels = c("", "C"),
                              nrow = 2,
                              font.label = list(size = 10)) 
tcoi_fig
...
tcomparison <- sensitivity(coi = 2:20, plaf = p, 
                           ...
                           distance = "squared", repetitions = 100, 
                           seq_error = 0.01)
...
tdistance <- sensitivity(coi = 2:20, plaf = p, comparison = "overall",
                         ...
                         repetitions = 100, coi_method = c("1", "2"), 
                         seq_error = 0.01)
...
tcoverage_1 <- sensitivity(coi = 2:20, coverage = c(50, 100, 250, 500, 1000, 2000), 
                         repetitions = 100, seq_error = 0.01,
                         plaf = p, coi_method = "1")

tcoverage_image_1 <- 
  sensitivity_plot(data = tcoverage_1,
                   result_type = "disc",
                   title = "Predicted COI",
                   dims = c(2, 3),
                   sub_title = paste0("Coverage ", c(50, 100, 250, 500, 1000, 2000)))

tcoverage_error_1 <- error_plot(tcoverage_1, fill = "coverage", 
                              legend_title = "Coverage", 
                              title = "Error")

tcoverage_fig_1 <- ggpubr::ggarrange(tcoverage_image_1, tcoverage_error_1, 
                                   labels = c("", "G"), 
                                   nrow = 2, 
                                   heights = c(5, 2),
                                   font.label = list(size = 10))
...
tcoverage_2 <- sensitivity(coi = 2:20, coverage = c(50, 100, 250, 500, 1000, 2000), 
                         repetitions = 100, seq_error = 0.01,
                         plaf = p, coi_method = "2")

tcoverage_image_2 <- 
  sensitivity_plot(data = tcoverage_2,
                   result_type = "disc",
                   title = "Predicted COI",
                   dims = c(2, 3),
                   sub_title = paste0("Coverage ", c(50, 100, 250, 500, 1000, 2000)))

tcoverage_error_2 <- error_plot(tcoverage_2, fill = "coverage", 
                              legend_title = "Coverage", 
                              title = "Error")

tcoverage_fig_2 <- ggpubr::ggarrange(tcoverage_image_2, tcoverage_error_2, 
                                   labels = c("", "G"), 
                                   nrow = 2, 
                                   heights = c(5, 2),
                                   font.label = list(size = 10))
...
knitr::include_graphics('figures/discrete/tcoverage_1.png')
knitr::include_graphics('figures/discrete/tcoverage_2.png')
```
...
# Add a loci column
tloci_1$boot_error$loci <- rep(c(1e2, 1e3, 1e4), 
                             each = length(unique(tloci_1$boot_error$coi)))
tloci_error_1 <- error_plot(tloci_1, fill = "loci", 
                            legend_title = "Loci", 
                            title = "Error")

tloci_fig_1 <- ggpubr::ggarrange(tloci_image_1, tloci_error_1, 
                                 labels = c("", "D"), 
                                 nrow = 2, 
                                 font.label = list(size = 10))
tloci_fig_1
...
# Add a loci column
tloci_2$boot_error$loci <- rep(c(1e2, 1e3, 1e4), 
                             each = length(unique(tloci_2$boot_error$coi)))
tloci_error_2 <- error_plot(tloci_2, fill = "loci", 
                            legend_title = "Loci", 
                            title = "Error")

tloci_fig_2 <- ggpubr::ggarrange(tloci_image_2, tloci_error_2, 
                                 labels = c("", "D"), 
                                 nrow = 2, 
                                 font.label = list(size = 10))
tloci_fig_2
...
tover_fig <- ggpubr::ggarrange(tover_image, tover_error, 
                               labels = c("", "K"), 
                               nrow = 2, 
                               heights = c(5, 2),
                               font.label = list(size = 10))
...
trelated <- sensitivity(coi = 2:20, relatedness = seq(0, 0.5, 0.1), 
                        repetitions = 100, seq_error = 0.01, 
                        plaf = p, coi_method = c("1", "2"))

trelated_image <- sensitivity_plot(data = trelated,
                                   result_type = "disc",
                                   title = "Predicted COI",
                                   dims = c(4, 3),
                                   sub_title = paste(paste0("Related =  ", seq(0, 0.5, 0.1)),
                                     rep(c("Method 1", "Method 2"), each = 6),
                                     sep = ", "))

trelated_error <- error_plot(trelated, fill = "relatedness", 
                          legend_title = "Related", 
                          title = "Error",
                          second_fill = "coi_method")

trelated_fig <- ggpubr::ggarrange(trelated_image, trelated_error, 
...
tepsilon <- sensitivity(coi = 2:20, epsilon = seq(0, 0.010, 0.005), 
                        repetitions = 100, seq_error = 0.01, plaf = p)
...
tseq <- sensitivity(coi = 2:20, epsilon = 0.1, 
                    seq_error = seq(0.1, 0.16, 0.02),
                    repetitions = 100, coi_method = "2", plaf = p)

# ep_title <- rep(paste0("Epsilon=", seq(0, 0.010, 0.005), 2))
# seq_title <- rep(paste0("Sequence Error = ", seq(0, 0.2, 0.05)[-1]), each = 3)
...
tseq_fig <- ggpubr::ggarrange(tseq_image, tseq_error, 
                              labels = c("", "M"), 
                              heights = c(5, 2),
                              nrow = 2, 
                              font.label = list(size = 10))
...
tbin <- sensitivity(coi = 2:20, bin_size = seq(10, 100, 30),
                    distance = "squared", plaf = p, repetitions = 100,
                    coi_method = c("1", "2"), seq_error = 0.01)

tbin_image <- sensitivity_plot(data = tbin,
                               result_type = "disc",
                               title = "Predicted COI",
                               dims = c(2, 4),
                               sub_title = rep(paste("Bin Size =", seq(10, 100, 30)), 2))

tbin_error <- error_plot(tbin, fill = "bin_size", 
                         fill_levels = c("10", "40", "70", "100"),
                         legend_title = "COI Method", 
                         title = "Error")

tbin_fig <- ggpubr::ggarrange(tbin_image, tbin_error, 
                              labels = c("", "C"), 
                              nrow = 2, 
                              font.label = list(size = 10)) 
tbin_fig
...
knitr::include_graphics('figures/discrete/tbin.png')
```

```{r save results, eval = eval_tests, echo = FALSE}
# Below, we save the runs we performed for future use
saved_analysis <- list(toverall, tcoi, tcomparison, tdistance, tcoverage, 
