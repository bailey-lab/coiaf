...
```{r}
tcoi <- coi_test(COI = 2:10, repetitions = 10,
                 PLAF = p, method = "overall", dist_method = "squared")
tcoi$error_bias

haha = tcoi$predicted_coi %>% tidyr::gather(true_COI, estimated_COI) %>%
  tidyr::extract(true_COI, c("true_COI", "loop_number"), "coi_(.+)_(.+)") %>%
  dplyr::mutate_all(as.numeric)

for (i in unique(haha$loop_number)){
  assign(paste("p", i, sep = ""),
         ggplot(dplyr::filter(haha, loop_number == i), aes(x = true_COI, y = estimated_COI)) +
           ...
           labs(x = "True COI", y = "Estimated COI"))
}
...
haha2 = tcoi_range$predicted_coi %>% tidyr::gather(true_COI, estimated_COI) %>% 
  tidyr::extract(true_COI, c("true_COI", "loop_number"), "coi_(.+)_(.+)") %>% 
  dplyr::mutate_all(as.numeric)

plot_data_column = function (data, loop_num, change_param = NULL, change_param_val = NULL){ 
  ggplot(dplyr::filter(data, loop_number == loop_num), 
...
uniq = unique(haha2$loop_number)
myplots <- lapply(uniq, plot_data_column, data = haha2, change_param = "COI Range", change_param_val = 10:11)


myplots2 <- lapply(unique(haha$loop_number), plot_data_column, data = haha)
