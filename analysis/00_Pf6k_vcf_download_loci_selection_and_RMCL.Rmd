...
We'll continue by subsetting to the SNPs that have a minor allele frequency greater than 0.05.

```{r}

# frequency filter
bcf_allele_frequency <- function(vcf, minor = 0.05) {
...
# submit filter 
pars <- data.frame("vcf" = l, "minor" = 0.05, stringsAsFactors = FALSE)
sopt <- list(time = '4:00:00', mem = "16Gb")
...
Now processing to calculate the intercepting SNPs between regionally MAF>0.05 and global MAF>0.05
...
ggplot(inclusions, aes(x=regions, y=loci)) + geom_point() + geom_line()
...
saveRDS(intersect_snps, file.path(here::here(),"analysis/data/derived/McCOIL_work/intercept_snps.rds"))
```
...
intersect_snps <- readRDS(
  file.path(here::here(),"analysis/data/derived/McCOIL_work/intercept_snps.rds")
  )
