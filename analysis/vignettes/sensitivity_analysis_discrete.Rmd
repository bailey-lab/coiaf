...
toverall <- cont_sensitivity(COI = 1:25, max_COI = 30, repetitions = 100, PLAF = p,
...
knitr::include_graphics('figures/toverall.png')
```
...
knitr::include_graphics('figures/tcoi.png')
```
...
tmethod <- sensitivity(COI = 2:20, PLAF = p, 
...
tmethod_fig <- ggpubr::ggarrange(tmethod_image, tmethod_error, labels = "AUTO") 
tmethod_fig
```

```{r method image, eval = eval_images, echo = F, out.width = "75%"}
knitr::include_graphics('figures/tmethod.png')
```
...
knitr::include_graphics('figures/tdistance.png')
```

### Weighted
```{r weight, eval = eval_tests}
tweight <- sensitivity(COI = 2:20, weighted = c(FALSE, TRUE),
                    dist_method = "squared", PLAF = p, repetitions = 100,
                    coi_method = c("1", "2"))
...
knitr::include_graphics('figures/tweight.png')
```
...
knitr::include_graphics('figures/tcoverage.png')
```
...
knitr::include_graphics('figures/tloci_1.png')
```
...
knitr::include_graphics('figures/tloci_2.png')
```
...
knitr::include_graphics('figures/talpha_1.png')
```
...
knitr::include_graphics('figures/talpha_2.png')
```
...
knitr::include_graphics('figures/tover.png')
```
...
knitr::include_graphics('figures/tepsilon.png')
```
...
knitr::include_graphics('figures/tseq.png')
```
...
saved_analysis <- list(toverall, tcoi, tmethod, tdistance, tweight, tcoverage, 
                       tloci_1, tloci_2, talpha_1, talpha_2,  tover, tepsilon, 
                       tseq)

names(saved_analysis) <- list("toverall", "tcoi", "tmethod", "tdistance", 
                              "tweight", "tcoverage", "tloci_1", "tloci_2", 
                              "talpha_1", "talpha_1", "tover", "tepsilon", 
                              "tseq")

saveRDS(saved_analysis, file = "saved_analysis.rds")

# Here, we update all the stored images
update_images <- list(toverall_fig, tcoi_fig, tmethod_fig, tdistance_fig, 
                      tweight_fig, tcoverage_fig, tloci_fig_1, tloci_fig_2, 
                      talpha_fig_1, talpha_fig_2, tover_fig, tepsilon_fig, 
                      tseq_fig)
names(update_images) <- list("toverall", "tcoi", "tmethod", "tdistance", 
                             "tweight", "tcoverage", "tloci_1", "tloci_2", 
                             "talpha_1", "talpha_2", "tover", "tepsilon", 
                             "tseq")

for (i in seq_len(length(update_images))){
  filename = paste0("analysis/vignettes/figures/", 
