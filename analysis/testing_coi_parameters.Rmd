...
```{r default table, eval = T, echo = F, results='asis'}
default_tabl <- '
...
knitr::include_graphics('Sensitivity Analysis Figures/tcoi_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tmethod_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tdistance_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tweight_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tcoverage_image.png')
```
...
# tcoverage <- coi_test(COI = 2:20, loci, 
#                  repetitions = 100, PLAF = p)
# 
# sensitivity_plot(data = tcoverage,
#                  plot_dims = c(2, 3),
...
knitr::include_graphics('Sensitivity Analysis Figures/talpha_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tover_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tepsilon_image.png')
```
...
knitr::include_graphics('Sensitivity Analysis Figures/tseq_image.png')
```
...
saved_analysis <- list(tcoi, tmethod, tdistance, tweight, tcoverage, talpha, 
                       tover, tepsilon, tseq)
saveRDS(saved_analysis, file = "analysis/saved_analysis.rds")

# Here, we update all the stored images
update_images <- list(tcoi_image, tmethod_image, tdistance_image, tweight_image,
                      tcoverage_image, talpha_image, tover_image, 
                      tepsilon_image, tseq_image)
names(update_images) <- list("tcoi_image", "tmethod_image", "tdistance_image",
                             "tweight_image", "tcoverage_image", "talpha_image", 
                             "tover_image", "tepsilon_image", "tseq_image")

for (i in seq_len(length(update_images))){
  filename = paste0("analysis/Sensitivity Analysis Figures/", 
