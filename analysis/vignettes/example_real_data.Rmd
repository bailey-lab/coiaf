...
coi_pred <- lapply(seq_len(length(example_real_data)), function(x) {
  ...
  coi_region <- lapply(seq_len(nrow(sample)), function(i) {
    ...
    coi_region <- optimize_coi(input, "real") 
  }) %>% unlist()
  
  coi_pred <- tibble::tibble(name = rownames(sample),
                             prediction = coi_region,
                             file = names(example_real_data)[x])
})
...
# Plot
ggplot(coi_pred_tib, aes(prediction, fill = file)) + 
  ...
  theme(plot.title   = element_text(hjust = 0.5, size = 10),
        axis.title   = element_text(size = 8)) +
  guides(fill = FALSE) +
  labs(x = "Predicted COI", y = "Density", title = "Bar Plot of Predicted COI", fill = NULL)
```
